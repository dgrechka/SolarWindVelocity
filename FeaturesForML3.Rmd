...
testing <- features.2015.lagged.no_sd[1001:2000,]
```
...
prepareForTraining <- function(dataSet,simulated) {
  ...
  dataSet1 <- dataSet[,!((1:ncol(dataSet)) %in% grep('_m$|_mean$',names(dataSet)))]
...
trControl <- trainControl(verboseIter=T,number = 2)
...
for(i in 0:15) {
  prepared <- prepareForTraining(training.no_time,simulated);
  pcas <- getPCA(prepared)
  model <- train(velocity_mean ~ .,data = pcas$data,method="rf", verbose=T,trControl=trControl)
  ...
  sds <- c(sds,currentRMSE)
  sds.sqr.sum <- sds.sqr.sum + currentRMSE*currentRMSE;
  sds.acc <- c(sds.acc,sqrt(sds.sqr.sum))
  
    
  simulators[[paste0('offset.',(i+1)*3)]] <- list(
    predictorNames = names(prepared)[names(prepared) != 'velocity_mean'],
...
```{r outputing}
write.csv(file='ResultData/MLSimulation.csv',row.names = F,results)
```