...
I am going to perform data clean up and feature extraction for later wind speed model fitting.
The major predictor of the solar wind is considered to be coronal holes (e.g. see <https://www.cfa.harvard.edu/~scranmer/Preprints/cranmer_y10pre.pdf>)

# Available data sets

I've got two CSV datasets to be used as training data for the modelling.
They contain quantitive features extracted from the Sun images using computer vision algorithms.
...
First, we will analyse the ch_2015_0193.dsv
...
We need to parse time field to numeric axis. I will use the hours offset from the first measurement.
...
For this study we will simple omit the observations with missing values

Befor omiting there were **`r nrow(ch_2015_0193)`** observations.

```{r missing values}
ch_2015_0193 <- na.omit(ch_2015_0193)
```

Afer omiting there are **`r nrow(ch_2015_0193)`** observations left.
...
Some of the variables are highly skewed. We will log-transform these varaibles.
...
For basic visual timeseries exlorations we will investigate first 2 month of data
...
As *Total_Area_CH_m* is fully correlated to *Relative_CH_Area_m* and *Total_CorrectShpereArea_CH_m* is fully correlated to *Relative_CH_CorrectSphereArea_m* we will leave only *Relative* varaibles.
...
Finnaly we have **`r length(consistent_variables)`** predictors that can be used for modell fitting.
(Time series are visualised later)

# The same transformations with another spectrum data
...
chPreprocess <- function (data) {
  ...
  data2$ts <- (data2$ts_sec -base_ts_sec)/3600;#hours sine basetime
...
color_scale <- c('col_193_apx'='red','col_211_apx'='green')
color_scale_labels <- c('col_193_apx'='approximated (193)','col_211_apx'='approximated (211)')
shape_scale <- c('shape_193_raw'=1,'shape_211_raw'=3)
shape_scale_labels <- c('shape_193_raw'='raw data (193)','shape_211_raw'='raw data (211)')

for(name in consistent_variables) {
  print(
    ggplot(ch_2015_0193_short) +
