...
I've got two CSV data sets to be used as training data for the modelling.
...
First, we will analyse features extracted from blue images ch_2015_0193.dsv
...
chJoin <- function(data1,data2) {
  left <- min(data1$ts,data2$ts)
  right <- max(data1$ts,data2$ts)
  ...
  for(name in consistent_variables) {
    ...
    sp1 <- smooth.spline(data1$ts,data1[[name_m]])
    sp2 <- smooth.spline(data2$ts,data2[[name_m]])
...
```{r export}
write.csv(file='CH_features_cleaned.csv',row.names = F,
          data.frame(ts=joint$ts,Relative_CH_CorrectSphereArea_j=joint$Relative_CH_CorrectSphereArea_j))
```
