...
for(name in noisy_vars) {
  print(
    ggplot(ch_2015_0193, aes_string('ts', name)) +
      geom_point(shape=1) 
      #geom_line(aes_string('ts',paste0(name,'_gam')),col='red')
  )
}
```
...
for(name in consistent_variables) {
  ...
  variable <- runmed(variable, k=17)
  variable <- runmed(variable, k=11)
  variable <- runmed(variable, k=5)
  ch_2015_0193[[paste0(name,'_m')]] <- variable
}
```
...
for(name in consistent_variables) { 
  variable <- ch_2015_0193[[paste0(name,'_m')]]  
  #ppcub <- ppfit(ts_var, variable, knots, method = "cubic")
  #ch_2015_0193[[paste0(name,'_ms')]] <- ppval(ppcub, ts_var)
}
```
...
for(name in consistent_variables) {
  print(
    ggplot(ch_2015_0193) +
      geom_point(aes_string('ts',name),shape=1)+
      #geom_point(aes_string('ts',paste0(name,'_m')),shape=1)
      geom_line(aes_string('ts',paste0(name,'_m')),col='red',lwd=1) 
  ) 
}
```
